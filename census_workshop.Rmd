...
doctoral_degrees_pct<-get_acs(geography="county",
                          variables=c("B15003_025", "B15003_001"),
                          output="wide",
                          year=2018) %>% 
                  rename(doctorate_holders_est=B15003_025E) %>% 
                  rename(doctorate_holders_moe=B15003_025M) %>% 
                  rename(over25_pop_est=B15003_001E) %>% 
                  rename(over25_pop_moe=B15003_001M) %>% 
                  mutate(pct_doctorate=(doctorate_holders_est/over25_pop_est)*100) %>% 
                  arrange(desc(doctorate_holders_est))

View(doctoral_degrees_pct)
        
```

```{r,echo=FALSE}
kable(head(doctoral_degrees_pct))
```


```{r}
medianincome_doctorates_county<-full_join(median_income,doctoral_degrees_pct,by="GEOID")
View(medianincome_doctorates_county)
```

```{r,echo=FALSE}
kable(head(medianincome_doctorates_county))
```
